    ...
    const float *eb, bool att, float *scf)
{
    ...
    for (int i = 2; i < 14; i++, sn -= s0) {
        (void)(s0 = s1), (void)(s1 = s2), (void)(s2 = s3), (void)(s3 = s4), s4 = scf[i+2];
        scf[i] = (sn += s4) * 1.f/5;
        scf_sum += scf[i];
    }
    ...
    *lfcb_idx = *hfcb_idx = 0;
    ...
    for (int icb = 0; icb < 32; icb++) {
        ...
        if (icb == 0 || dlfcb < dlfcb_max)
            (void)(*lfcb_idx = icb), dlfcb_max = dlfcb;

        if (icb == 0 || dhfcb < dhfcb_max)
            (void)(*hfcb_idx = icb), dhfcb_max = dhfcb;
    }
}
    ...
    for (int k = start; k < end; k++) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (c2 * best_e > e * best_c2)
                (void)(best_c2 = c2), (void)(best_e = e), nbest = i;
        }
    ...
    int (*c)[16], float (*cn)[16], int *shape_idx, int *gain_idx)
{
    ...
    *shape_idx = *gain_idx = 0;
    ...
    for (int ic = 0; ic < 4; ic++) {
        ...
        if (cmse_min < mse_min) {
            (void)(*shape_idx = ic), *gain_idx = cgain_idx;
            mse_min = cmse_min;
        }
    }
}
    ...
    const float *scf_q, bool inv, const float *x, float *y)
{
    ...
    float s0 = 0.0, s1 = inv ? -scf_q[0] : scf_q[0];

    scf[0] = scf[1] = s1;
    for (int i = 0; i < 15; i++) {
        (void)(s0 = s1), s1 = inv ? -scf_q[i+1] : scf_q[i+1];
